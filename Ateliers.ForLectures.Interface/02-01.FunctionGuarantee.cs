...
namespace Ateliers.ForLectures.Interface.FunctionGuarantee
{
    ...
    public class FunctionGuarantee
    {
        ...
        「同じメソッドやプロパティを保証するなら、別クラスに定義し、継承するのではダメなのですか？」

        この質問は、インターフェースと抽象(基底)クラスの違いを理解していない人にとっては、ごく自然な疑問だと思います。

        この質問に対しては
        「異なる継承元であっても、同じインターフェースを定義していれば同様に扱うことができ、リストでまとめたり、引数に渡すことができる」
        という形で説明するのが、最もわかりやすく納得してもらえる説明だと思います。

        1. インターフェースと抽象クラスの違い

        インターフェースは、何をすべきかを定義するものです。
        一方、抽象クラスは、どのようにすべきかを定義するものです。

        インターフェースは、実装するクラスに対してメソッドの定義を強制することはできません。（C# 8.0）
        一方、抽象クラスは、継承するクラスに対してメソッドの定義を強制することができます。

        インターフェースは、単一継承と多重継承の両方をサポートしています。
        一方、抽象クラスは単一継承しかサポートしていません。

        2. インターフェースの利点

        インターフェースを使用することで、異なる種類のオブジェクトを共通の基盤で扱うことができます。
        インターフェースを使用することで、コードの再利用性と保守性を向上させることができます。
        インターフェースを使用することで、疎結合なコードを書くことができます。
         ----------------------------------------------------------------------------------------------------------------------------------------------------------- */
        #endregion
